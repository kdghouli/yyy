...
<div>
  ...
  <p class="pt-2">Nombre des <span class="fw-bolder"><span><i class="bi bi-truck text-success fs-4"></i></span> Camions:
  ...
  <div class="row row-cols-lg-3 row-cols-md-2 ">
    ...
    <ul v-for="camion in camionsMat" class="list-group"  :key="camion">
      ...
      <li
        ...
        <router-link class="text-decoration-none" :to="{name:'CamionView',params:{id:camion.id}}">{{ camion.matricule}}
        </router-link>
        <span class="fw-lighter ms-4 me-auto align-bottom" style="font-size: 9px">{{ camion.intitule }}</span> <span
          class="badge bg-success rounded-pill" v-if="calcComments(camion.id)">{{ calcComments(camion.id) }}</span>
      </li>
    ...
    <ul v-for="voiture in voituresMat" class="list-group" :key="voiture">
      ...
      <li
        ...
        <router-link class="text-decoration-none" :to="{name:'VoitureView',params:{id:voiture.id}}">{{
          voiture.matricule}}</router-link> <span class="fw-lighter ms-2" style="font-size: 9px">{{ voiture.type
          }}&nbsp;&nbsp;&nbsp;&nbsp;{{ voiture.conducteur }}</span> <span class="badge bg-primary rounded-pill">4</span>
      </li>
  ...
  <p class="pt-2">Nombre des <span class="fw-bolder"><img class="me-1" src="./../../public/forklift.png" alt=""
        style="width: 30px;" /> Chariots </span> <span class="badge rounded-pill bg-warning ms-1">{{ chariotsMat.length
      }}
      </span> </p>
    ...
    <ul v-for="chariot in chariotsMat" class="list-group" :key="chariot">
      ...
      <li
        ...
        <router-link class="text-decoration-none" :to="{name:'ChariotView',params:{id:chariot.matricule}}">{{
          ...
          style="font-size: 9px">{{ chariot.marque }}</span><span class="badge bg-warning rounded-pill">12</span>
      </li>
  ...
  <p class="pt-2">Nombre des <span class="fw-bolder"><img class="me-1" src="./../../public/scooter.png" alt=""
        style="width: 30px;" /> Scooters : </span><span class="badge rounded-pill bg-danger ms-1">{{ scootersMat.length
      }}</span> </p>
    ...
    <ul v-for="scooter in scootersMat" class="list-group" :key="scooter">
      ...
      <li
        ...
        <router-link class="text-decoration-none" :to="{name:'ScooterView',params:{id:scooter.matricule}}">{{
          ...
          style="font-size: 10px">{{ scooter.utilisateur }}</span><span class="badge bg-danger rounded-pill">15</span>
      </li>
